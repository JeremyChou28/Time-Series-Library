...
class Model(nn.Module):
    ...
    def MLP_temporal(self, x, B, N, L):
        # [B, N, T, D]
        x = torch.fft.rfft(x, dim=2, norm="ortho")  # FFT on L dimension    L//2+1
    ...
    def MLP_channel(self, x, B, N, L):
        ...
        x = x.permute(0, 2, 1, 3)
        # [B, T, N, D]
        x = torch.fft.rfft(x, dim=2, norm="ortho")  # FFT on N dimension    # N//2+1
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        if (
            ...
            return dec_out[:, -self.pred_len :, :]  # [B, T, N]
        else:
