...
class MIC(nn.Module):
    ...
    def conv_trans_conv(self, input, conv1d, conv1d_trans, isometric):
        ...
        x1 = self.drop(self.act(conv1d(x)))  # B,D,(2T-K+2*K//2)/S+1
        x = x1
    ...
    def forward(self, src):  # src: B,2T,D
        # multi-scale
        multi = []
...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        trend = self.regression(trend.permute(0, 2, 1)).permute(0, 2, 1)  # B,T,C
        ...
        dec_out = self.dec_embedding(seasonal_init_dec, x_mark_dec)  # B,2T,C
        dec_out = self.conv_trans(dec_out)  # B,2T,C
